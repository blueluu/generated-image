...
const getCallerProjectRoot = () => {
    // 优先使用环境变量中的项目路径
    const envProjectPath = process.env.USER_PROJECT_ROOT || process.env.PROJECT_ROOT;
    if (envProjectPath) {
...
async function getApiKey(providedApiKey) {
    if (providedApiKey) {
        // 如果用户提供了API Key，保存它
    ...
    throw new Error("请先设置ModelScope API密钥。使用 set_api_key 工具设置API密钥，或在调用时提供 api_key 参数。");
}
// 创建MCP服务器
    ...
    return {
        tools: [
            ...
            },
            {
                ...
                description: "设置用户项目根目录，图片将保存在此目录下的generated-images文件夹中",
                inputSchema: {
                    type: "object",
                    properties: {
                        project_root: {
    ...
    try {
        switch (name) {
            ...
            case "set_project_root":
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `✅ 项目根目录设置成功！\n📁 项目根目录: ${project_root}\n💾 图片将保存到: ${path.join(project_root, "generated-images")}`,
                        },
                    ],
                };
            ...
            case "generate_web_image":
                const { prompt, api_key, size_preset, width, height, filename } = args;
                if (!prompt) {
                ...
                // 处理中文prompt翻译
                const { translatedPrompt, isTranslated } = translateChinesePrompt(prompt);
                // 确定图片尺寸
                let finalWidth;
                let finalHeight;
                ...
                const imageBuffer = await imageService.generateImage(translatedPrompt, finalWidth, finalHeight);
                // 生成文件名
                const finalFilename = filename || `web-image-${uuidv4().slice(0, 8)}.jpg`;
                // 确保输出目录存在（使用绝对路径）
...
📊 文件大小: ${(imageBuffer.length / 1024).toFixed(2)} KB`,
            ...
            case "generate_multiple_images":
                const { images, api_key: batchApiKey } = args;
                if (!images || !Array.isArray(images) || images.length === 0) {
                    throw new Error("images数组不能为空");
                }
                // 获取API Key（优先使用提供的，否则使用存储的）
                const finalBatchApiKey = await getApiKey(batchApiKey);
                const batchImageService = new ImageGenerationService(finalBatchApiKey);
                const results = [];
                // 确保输出目录存在（使用绝对路径）
                ...
                for (let i = 0; i < images.length; i++) {
                    ...
                    try {
                        ...
                        const imgBuffer = await batchImageService.generateImage(imageConfig.prompt, imgWidth, imgHeight);
                        const imgFilename = imageConfig.filename ||
                        ...
                        const imgSizeInfo = imageConfig.size_preset
                            ? `${COMMON_SIZES[imageConfig.size_preset].description} (${imgWidth}x${imgHeight})`
                            : imgWidth && imgHeight
...
${results
        ...
        return {
            content: [
                {
                    type: "text",
                    text: `❌ 错误: ${error instanceof Error ? error.message : "Unknown error"}`,
                },
            ],
            isError: true,
        };
